...
- We have defined a custom role called "elasticsearch" in "ansible/playbooks/roles/elasticsearch" that is fully ready for customization and enhancements.
- We are heavily leveraging Roles created by Jeff Geerling (a.k.a. Geerlingguy: **geerlingguy.docker**)

1. Run the playbook in CHECK mode - which means changes to the host are simulated rather than actually performed on the end host.

~~~
ansible-playbook deploy_elasticsearch.yml -i inventory.yml --check
~~~

2. Debug Playbook Execution

~~~
ansible-playbook deploy_elasticsearch.yml -i inventory.yml --check -vvv
~~~

3. Syntax Check Mode is also an option or using the Ansible linter

~~~
ansible-playbook deploy_elasticsearch.yml -i inventory.yml --syntax-check
