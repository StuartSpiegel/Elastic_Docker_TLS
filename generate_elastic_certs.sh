#!/usr/bin/env bash
#
# Generates Elasticsearch certificates using elasticsearch-certutil (via Docker),
...
echo "CA_PASS:      ${CA_PASS:+"<set>"}"   # If blank, show empty
...
# Convert CERTS_DIR to an absolute path
...
if [[ ! -f "$CA_FILE" ]]; then
  ...
  docker run --rm \
    -v "$CERTS_DIR_ABS:/certs" \
    "${DOCKER_IMG_BASE}:${ES_VERSION}" \
    bash -c "echo '' | elasticsearch-certutil ca \
...
if [[ ! -f "$ES_NODE_P12" ]]; then
  ...
  docker run --rm \
    -v "$CERTS_DIR_ABS:/certs" \
    "${DOCKER_IMG_BASE}:${ES_VERSION}" \
    bash -c "echo '' | elasticsearch-certutil cert \
      --ca /certs/elastic-stack-ca.p12 \
      --ca-pass \"${CA_PASS}\" \
      --out /certs/elasticsearch-nodes.p12 \
      --pass \"${P12_PASS}\" \
      --dns ${NODE_DNS}"
...
# 3. (Optional) Generate Kibana certificate if requested
...
# 4. (Optional) Convert PKCS#12 to PEM
#    Uncomment if needed:
# echo "[Step 4/4] Converting PKCS#12 -> PEM"
# docker run --rm \
#   -v "$CERTS_DIR_ABS:/certs" \
#   "${DOCKER_IMG_BASE}:${ES_VERSION}" \
#   bash -c "openssl pkcs12 -in /certs/elasticsearch-nodes.p12 -out /certs/elasticsearch.crt.pem -clcerts -nokeys -password pass:${P12_PASS} &&
#            openssl pkcs12 -in /certs/elasticsearch-nodes.p12 -out /certs/elasticsearch.key.pem -nocerts -nodes -password pass:${P12_PASS}"

echo "================================================="
echo "Certificate Generation Complete."
echo "CA:            $CA_FILE"
echo "ES Node Cert:  $ES_NODE_P12"
[[ -n "$KIBANA_DNS" ]] && echo "Kibana Cert:    $KIBANA_P12"
echo "================================================="
